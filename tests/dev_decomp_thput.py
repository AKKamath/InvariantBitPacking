...
def transfer_host(tensor):
    tensor = tensor.to("cpu")
    torch.cuda.synchronize()
    start = time.time_ns()
    for i in range(ITERS):
        tensor_copy = tensor.to("cuda")
    torch.cuda.synchronize()
    end = time.time_ns()
    print(f"Transfer time: {(end - start) / 1e6 / ITERS:.3f}ms")
    tot_time = (end - start) / 1e6 / ITERS
...
NUM_VECS = 100000
#SIZES = [256, 400, 512]
index_arr = None#torch.arange(0, NUM_VECS, 4).to("cuda")
...
for size in SIZES:
    ...
    tensor = torch.zeros([NUM_VECS, size // 4], dtype=torch.int32).to("cuda")
    runtime[size][-1] = transfer(tensor)
    runtime[size][-2] = transfer_host(tensor)
    for rate in TARGET:

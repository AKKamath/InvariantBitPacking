...
void check(char *data, int64_t *indices, size_t element_size, size_t array_size, int *flag) 
{
    ...
    check_kernel <<< 1, 512 >>> (cache, data, indices, element_size, array_size, cache_size, flag);
...
#define RUN_TEST(function, name) \
    reset(); \
    START = TIME_NOW; \
    function(cpu_buffer, gpu_indices, i, array_size, 160, 1024); \
    END = TIME_NOW; \
...
#define ARR_SIZE 9
const long int array[] = {64, 128, 256, 448, 512, 1024, 2048, 4096, 8192, 16384};
